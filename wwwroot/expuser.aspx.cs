...
public partial class expuser : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                */
                ...
                //至此验证通过
                cmd = new OleDbCommand("select * from tReportUser where report_id='" + report_id + "'", cnn);
                datar = cmd.ExecuteReader();
                if (!datar.HasRows)
                {
                    cnn.Close();
                    throw new Exception("错误:report_user中未找到report中的对应项");
                }
                else
                {
                    datar.Read();
                    //这里取报表的用户类别,而不是sessin的
                    clsReport.PushReportUserInfoToClient(this, report_user_class , datar["report_name"].ToString(), datar["report_number"].ToString(), datar["report_contact"].ToString(), datar["report_phone"].ToString(),
                        datar["report_mobile"].ToString(), datar["report_bankcardname"].ToString(), datar["report_bankcardid"].ToString(), datar["report_bankname"].ToString());
                    cnn.Close();
